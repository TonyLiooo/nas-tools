...
class Subscribe:
    ...
    def __get_media_info(self, tmdbid, name, year, mtype, cache=True, subtitle=None):
        """
        综合返回媒体信息
        ...
        :param mtype: 媒体类型
        :param cache: 是否使用缓存
        :param subtitle: 副标题，用于解析剧集信息
        """
        if tmdbid and not str(tmdbid).startswith("DB:"):
            media_info = MetaInfo(title="%s %s".strip() % (name, year), subtitle=subtitle)
            tmdb_info = self.media.get_tmdb_info(mtype=mtype, tmdbid=tmdbid)
            media_info.set_tmdb_info(tmdb_info)
        else:
            media_info = self.media.get_media_info(title="%s %s" % (name, year), subtitle=subtitle, mtype=mtype, strict=True, cache=cache)
        return media_info
